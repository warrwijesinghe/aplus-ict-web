...
export const useStudentProfile = (enabled = true) => useQuery({ queryKey: keys.profile, queryFn: studentLearningApi.profile, enabled });
...
export const useUnenrollFromCourse = () => { const client = useQueryClient(); return useMutation({ mutationFn: studentLearningApi.unenroll, onSuccess: () => client.invalidateQueries({ queryKey: ['student'] }) }); };
