...
export const PublicFooter = () => {
  ...
  return <footer className="footer"><div className="footer-inner"><div className="footer-grid"><section className="footer-brand-column"><Link className="footer-brand" to="/"><BrandLogo brandName={data?.brandName} variant="light" /></Link><p>Structured online ICT learning for Sri Lankan students from Grade 6 to G.C.E. A/L.</p></section><section><h2>Learn</h2><nav aria-label="Learn"><Link to="/school-ict">Grades 6–9</Link><Link to="/ol-ict">O/L ICT</Link><Link to="/al-ict">A/L ICT</Link><Link to="/resources">Free Resources</Link></nav></section><section><h2>Student Support</h2><nav aria-label="Student support"><Link to="/student-guide">Student Guide</Link>{isAuthenticated ? <Link to={destinationForUser(user)}>My Learning</Link> : <a href={`${serviceUrls.auth}/api/v1/auth/google?returnTo=/`}>My Learning</a>}<Link to="/contact">Contact</Link></nav></section><section><h2>Contact</h2>{data?.contactChannels?.map((item) => <p className="footer-contact" key={item.id}>{item.publicUrl ? <a href={item.publicUrl}>{item.label}</a> : item.label}</p>)}<nav aria-label="Social links">{socialLinks.map((item) => <SocialLink item={item} key={item.id} />)}</nav></section></div>{data?.legalBusinessName ? <section className="footer-legal-bar"><div className="footer-legal-copy"><h2>Operated by</h2><p><strong>{data.legalBusinessName}</strong><br />Company Registration No: {data.companyRegistrationNumber}<br />{address?.line1}, {address?.line2}, {address?.city}, {address?.country}</p><p>{data.relationshipStatement}</p></div><nav aria-label="Legal policies" className="footer-policy-links"><Link to="/privacy-policy">Privacy Policy</Link><Link to="/terms">Terms and Conditions</Link><Link to="/refund-policy">Refund Policy</Link><Link to="/cancellation-policy">Cancellation Policy</Link><Link to="/payment-policy">Payment Policy</Link></nav></section> : null}<p className="footer-copyright">© 2026 Miracle Network and Solutions (Pvt) Ltd. All rights reserved.</p></div></footer>;
};
