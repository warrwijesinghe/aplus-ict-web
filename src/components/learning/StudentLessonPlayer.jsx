...
export const ActivityRenderer = ({ activity }) => {
  ...
  if (activity.isLocked) { const state = activity.accessState || {}; return <section className="player-lock" role="status"><h2>Locked activity</h2>{(state.reasons?.length ? state.reasons : ['This activity is not available yet.']).map((reason) => <p key={reason}>{reason}</p>)}{state.nextAvailableAt ? <p>Available from {new Date(state.nextAvailableAt).toLocaleDateString()}.</p> : null}{state.unmetRequirements?.some((item) => item.type === 'premium') ? <Link className="button secondary" to={`/courses/${courseSlug}/lessons/${lessonSlug}/exam-success-pack`}>View Exam Success Pack</Link> : null}</section>; }
