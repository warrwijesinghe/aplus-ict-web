...
export const ExamSuccessPackPage = () => {
  ...
  return <section className="exam-success-pack"><nav className="breadcrumbs"><Link to={`/courses/${courseSlug}/learn`}>My course</Link><span>/</span><Link to={`/courses/${courseSlug}/lessons/${lessonSlug}`}>{data.lesson.title}</Link><span>/</span><span>Exam Success Pack</span></nav><p className="eyebrow">Exam Success Pack</p><h1>{product.name}</h1><p>{product.shortDescription || product.description || `Premium practice and revision activities for ${data.lesson.title}.`}</p><div className="exam-pack-summary"><strong>{price(product.price, product.currency)}</strong><span>{data.includedPremiumActivityCount} premium activit{data.includedPremiumActivityCount === 1 ? 'y' : 'ies'} included</span><span>{product.entitlementDurationDays ? `${product.entitlementDurationDays} days of access` : 'No automatic expiry'}</span></div>{product.description ? <div className="player-rich-content">{product.description}</div> : null}{data.hasActiveEntitlement ? <p className="success-message">Your Exam Success Pack is active. Premium activities in this Lesson are unlocked.</p> : data.pendingOrder ? <section className="payment-pending"><h2>Payment pending</h2><p>Order {data.pendingOrder.orderNumber} is waiting for payment confirmation.</p><Link className="button" to={`/student/orders/${data.pendingOrder.id}`}>View order</Link></section> : <><p>Payment is not taken on this page. Creating an order reserves the server-calculated price and shows the current payment instructions.</p>{create.error ? <InlineError error={create.error} /> : null}<button className="button" disabled={create.isPending} onClick={() => create.mutate()} type="button">{create.isPending ? 'Creating order…' : 'Create order'}</button></>}</section>;
};
...
export const StudentOrderDetailPage = () => {
  ...
  const item = order.data; const cancellable = ['pending', 'payment_pending', 'awaiting_payment'].includes(item.status) && ['unpaid', 'pending'].includes(item.paymentStatus);
  return <section className="order-detail"><Link to="/student/orders">← My orders</Link><p className="eyebrow">Payment pending</p><h1>Order {item.orderNumber}</h1><p><strong>{item.status.replaceAll('_', ' ')}</strong> · Payment {item.paymentStatus}</p><dl><dt>Product</dt><dd>{item.items.map((row) => row.name).join(', ')}</dd><dt>Amount</dt><dd>{price(item.total, item.currency)}</dd><dt>Created</dt><dd>{date(item.createdAt)}</dd><dt>Next step</dt><dd>Online payment will be available in Task 15. Do not send payment until the supported payment instructions are shown.</dd></dl>{cancellable ? <button disabled={cancel.isPending} onClick={() => { if (window.confirm('Cancel this unpaid order?')) cancel.mutate(); }} type="button">{cancel.isPending ? 'Cancelling…' : 'Cancel unpaid order'}</button> : null}{cancel.error ? <InlineError error={cancel.error} /> : null}</section>;
};
