...
const renderWithProviders = (ui, auth = { isAuthenticated: false, user: null }) =>
  render(
    <MemoryRouter>
...
test('renders the focused Grades 6–13 homepage with one H1', () => {
  ...
  expect(
    screen.getByRole('heading', { level: 1, name: /study ict anytime. anywhere/i })
  ).toBeInTheDocument();
  expect(screen.getAllByRole('heading', { level: 1 })).toHaveLength(1);
  expect(screen.getByRole('link', { name: 'Start ICT Learning Now Free!' })).toHaveAttribute(
    'href',
    '/login'
  );
});

test('shows signed-in students the three ICT learning pathways in the hero', () => {
  renderWithProviders(<PlatformHomePage />, {
    isAuthenticated: true,
    user: { name: 'Student', roles: [{ code: 'student' }] }
  });

  const hero = document.querySelector('.platform-home-hero');
  expect(within(hero).getByRole('link', { name: 'A/L ICT' })).toHaveAttribute('href', '/al-ict');
  expect(within(hero).getByRole('link', { name: 'O/L ICT' })).toHaveAttribute('href', '/ol-ict');
  expect(within(hero).getByRole('link', { name: 'Grades 6–9 ICT' })).toHaveAttribute(
    'href',
    '/school-ict'
  );
  expect(
    within(hero).queryByRole('link', { name: 'Start ICT Learning Now Free!' })
...
test('renders the learning-freedom sections between course collections with their guest CTA destinations and accessible imagery', async () => {
  ...
  await waitFor(() =>
    expect(
      screen.getByRole('heading', { level: 2, name: 'Study Anytime, From Your Own Place' })
    ).toBeInTheDocument()
  );
  expect(
    screen.getByRole('heading', { level: 2, name: 'Study on the Device You Already Use' })
  ).toBeInTheDocument();
  expect(
    screen.getByRole('heading', { level: 2, name: 'Learn at Your Own Speed, in Your Own Way' })
  ).toBeInTheDocument();
  ...
  expect(within(anywhereSection).getByRole('link', { name: 'A/L ICT' })).toHaveAttribute(
    'href',
    '/al-ict'
  );
  expect(within(anywhereSection).getByRole('link', { name: 'O/L ICT' })).toHaveAttribute(
    'href',
    '/ol-ict'
  );
  expect(within(anywhereSection).getByRole('link', { name: 'Grades 6–9 ICT' })).toHaveAttribute(
    'href',
    '/school-ict'
  );
...
test('groups homepage courses by school stage in database display order', async () => {
  vi.spyOn(contentApi, 'publicCourses').mockResolvedValue({
    data: [
      {
        id: 'school-2',
        courseGroup: 'SCHOOL',
        medium: { code: 'en', nameEn: 'English Medium' },
        sortOrder: 20,
        title: 'Grade 9 ICT',
        slug: 'grade-9-ict-en'
      },
      {
        ...
        medium: { code: 'en', nameEn: 'English Medium' },
        sortOrder: 20,
        title: 'A/L ICT - Part 2',
        slug: 'al-ict-en-part-2'
      },
      {
        id: 'ol-1',
        courseGroup: 'OL',
        medium: { code: 'en', nameEn: 'English Medium' },
        sortOrder: 10,
        title: 'O/L ICT - Part 1',
        slug: 'ol-ict-en-part-1'
      },
      {
        id: 'ol-2',
        courseGroup: 'OL',
        medium: { code: 'en', nameEn: 'English Medium' },
        sortOrder: 20,
        title: 'O/L ICT - Part 2',
        slug: 'ol-ict-en-part-2'
      },
      {
        id: 'ol-3',
        courseGroup: 'OL',
        medium: { code: 'en', nameEn: 'English Medium' },
        sortOrder: 30,
        title: 'O/L ICT - Part 3',
        slug: 'ol-ict-en-part-3'
      },
      {
        id: 'al-1',
        courseGroup: 'AL',
        medium: { code: 'en', nameEn: 'English Medium' },
        sortOrder: 10,
        title: 'A/L ICT - Part 1',
        slug: 'al-ict-en-part-1'
      },
      {
        id: 'school-1',
  ...
  renderWithProviders(<PlatformHomePage />);
  await waitFor(() =>
    expect(screen.getByRole('heading', { level: 2, name: 'A/L ICT Courses' })).toBeInTheDocument()
  );
  ...
  ).toEqual(['A/L ICT - Part 1', 'A/L ICT - Part 2']);
  expect(
    within(olSection).getByRole('heading', { level: 3, name: 'O/L ICT - Part 1' })
  ).toBeInTheDocument();
  expect(
    within(schoolSection)
      .getAllByRole('heading', { level: 3 })
      .map((heading) => heading.textContent)
  ).toEqual(['Grade 6 ICT', 'Grade 9 ICT']);
  ...
  expect(within(alSection).getByRole('link', { name: 'View all A/L ICT Courses' })).toHaveAttribute(
    'href',
    '/al-ict'
  );
  expect(within(olSection).getByRole('link', { name: 'View all O/L ICT Courses' })).toHaveAttribute(
    'href',
    '/ol-ict'
  );
  expect(
    within(schoolSection).getByRole('link', { name: 'View all Grades 6–9 ICT Courses' })
  ).toHaveAttribute('href', '/school-ict');
  const gradeSixCard = within(schoolSection)
    .getByRole('heading', { level: 3, name: 'Grade 6 ICT' })
    .closest('article');
  expect(within(gradeSixCard).getByRole('link', { name: 'View Course' })).toHaveAttribute(
    'href',
    '/courses/grade-6-ict-en'
  );
});
...
test('shows the four essential FAQ questions and educator identity', () => {
  ...
  expect(screen.getByRole('link', { name: /learn about a plus ict/i })).toHaveAttribute(
    'href',
    '/about'
  );
});
